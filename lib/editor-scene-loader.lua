...
local function propertyListToKeyValue(list, map)
	...
	for _,prop in ipairs(list) do
		if prop.isExtra then  table.insert(nonBuiltinProps, prop)
		else  mapPropertyValue(map, prop.name, prop.type, prop.value)  end
	end
	if nonBuiltinProps[1] then
		local extraProps = {}
		map.properties = extraProps
		for _,prop in ipairs(nonBuiltinProps) do
			mapPropertyValue(extraProps, prop.name, prop.type, prop.value)
		end
	end
end
